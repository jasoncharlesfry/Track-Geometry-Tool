...
void UART_Init(void){
  ...
  UART5_IBRD_R = 520;	//43;                    // IBRD = int(80,000,000 / (16 * 115,200)) = int(43.403)
  UART5_FBRD_R = 53;		//26;                    // FBRD = round(0.4028 * 64 ) = 26
                                        // 8 bit word length (no parity bits, one stop bit, FIFOs)
  UART5_LCRH_R = (UART_LCRH_WLEN_8|UART_LCRH_FEN);
  UART5_CTL_R |= UART_CTL_TXE | UART_CTL_RXE | UART_CTL_UARTEN;       // enable UART
  
                                        // configure PA1-0 as UART
  GPIO_PORTE_PCTL_R      = (GPIO_PORTE_PCTL_R & 0xFF00FFFF) + 0x00110000;
  GPIO_PORTE_AMSEL_R &= ~0x30;          // disable analog functionality on P%
